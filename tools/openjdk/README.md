...
### Excluding a Java class also excludes its generated header

`java.desktop/Java.gmk` drops `unix/classes/sun/font/*.java` for android, the
same way macOS does. `libfontmanager` then stopped on:

```
X11FontScaler.c:42:10: fatal error: 'sun_font_NativeFont.h' file not found
```

That header is generated by javac from `sun.font.NativeFont`'s native methods.
Excluding the class removes the header, and `X11FontScaler.c` includes it -
but android was falling into the `else` branch of
`LIBFONTMANAGER_EXCLUDE_FILES`, the linux one, which keeps that file.

macOS excludes both halves, and always has. This port had done only the Java
one. Putting android in the macosx branch pairs them again:

| target | excluded from libfontmanager |
| ------ | ---------------------------- |
| linux, aix | `fontpath.c lcdglyph.c` |
| windows | `X11FontScaler.c X11TextRenderer.c` |
| macosx, **android** | `X11FontScaler.c X11TextRenderer.c fontpath.c lcdglyph.c` |

Worth stating as a rule, because it is a whole class of failure and this port
has now hit it once: **whenever this patch excludes Java sources, the native
sources that include their generated JNI headers have to go with them.** A
sweep for the remaining cases - every C file including a
`sun_awt_X11*`, `sun_java2d_x11*`, `sun_java2d_xr*`, `sun_java2d_opengl*`,
`sun_font_NativeFont` or `sun_font_X11TextRenderer` header - finds only these
two plus the `unix/native/common/java2d/{x11,opengl}` files already excluded
above, and the shared `java2d/opengl` sources, which are guarded from line 26,
before their includes, and so never reach them under HEADLESS.
